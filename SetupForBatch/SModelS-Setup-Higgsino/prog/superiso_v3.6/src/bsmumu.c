...
/* computes the branching ratio of Bd -> l+ l- */
{
	...
	*((1.-4.*ml*ml/param->m_Bd/param->m_Bd)*pow(param->m_Bd/(param->mass_b_pole+param->mass_d)*cabs(CQ1),2.) + pow(cabs(param->m_Bd/(param->mass_b_pole+param->mass_d)*CQ2+2.*C10*ml/param->m_Bd),2.));
}
...
{
	return Bdll(2,C0b,C1b,C2b,CQ0b,CQ1b,param,mu_b);
}
