...
def calculation(part_num, total_parts, file_addon, delete_data, data_store_num=100):
    ...
    files = get_file_list(csv_file_path, int(part_num), int(total_parts))
    ...
    for k, element in enumerate(files):
        if k%data_store_num==0: compress_data('dataset_' + str(file_addon) + '_' + str(part_num) + '_' + str(total_parts) + '_' + str(int(k/data_store_num)))
    ...
    compress_data('dataset_' + str(file_addon) + '_' + str(part_num) + '_' + str(total_parts) + '_' + str(data_store_num) + '_fin')
