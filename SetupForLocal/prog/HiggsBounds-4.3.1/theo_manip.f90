...
!      if(debug)
     write(*,*) 'warning: sum of BR for '//trim(adjustl(pdesc(Hneut)%long))//' at line number=',jj,'is',testsumBR
    endif
   enddo
   deallocate(testBR)
  endif 
...
&            + sum(abs( partR(n)%qq_hjWm(:,i) - partR(n)%qq_hjWp(1,i))) )/partR(n)%qq_hjWp(1,i)) .lt. 1.0D-5 )then
   ...
   theo(n)%gg_hj_ratio(i) = partR(n)%gg_hj(i)
   theo(n)%bb_hj_ratio(i) = partR(n)%qq_hj(5,i)
