...
def run_SModelS(interHiggs):
    ...
    os.chdir(para.pathprog_SModelS)
    try:
        subprocess.check_call('runSModelS.py -f ' + dir_name + file_name + ' -p parameters.ini -o ' + dir_name + ' -v warning > ' + dir_name + logfile_name + ' 2>&1', shell=True)
    except:
...
def check_valid(interHiggs):
    ...
    try:
        ...
        input = pyslha.readSLHAFile(dir_name + file_name + '.smodelsslha')
    try:
        ex = int(input.blocks['SMODELS_EXCLUSION'][0,0])
    except:
